...
<section>
                            ...
                            <div class="search-box">
                                <div class="position-relative">
                                    <input id="search" name="search" type="text" class="form-control"
                                        autocomplete="off" id="searchTableList" placeholder="Search size..."
                                        ...
                                        <tr>
                                            <td>
                                                <input type="checkbox" id="checkedItems_{{ $index }}"
                                                    ...
                                                    value="{{ $size->id }}" wire:model="checkedItems"
                                                    wire:change="updateFilter">
                                            </td>
                                            <td>
                                                <h5 class="font-size-14 text-dark text-truncate mb-1">
                                                    {{ $size->name }}</h5>
                                                <span>Code : <span class="fw-medium">{{ $size->code }}</span></span>
                                            </td>
                                            <td>
                                                <span
                                                    class="badge badge-pill font-size-11 @if ($size->visibility == '1') badge-soft-danger @else badge-soft-success @endif">
                                                    {{ $size->visibility == '0' ? 'Publish' : 'Private' }}
                                                </span>
                                            </td>
                                            <td>
    ...
    @include('livewire.admin.sizes.add')
    @include('livewire.admin.sizes.edit')
    @include('livewire.admin.sizes.delete')
    @include('livewire.admin.sizes.selected')
</section>
